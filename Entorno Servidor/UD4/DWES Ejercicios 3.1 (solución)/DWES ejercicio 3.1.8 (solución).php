	...
	<nav></nav>
        <main>
			...
			if (isset($_REQUEST['enviar'])) { // Se ha recibido código y nombre del cliente y se procede a obtener y mostrar información de sus pedidos.
				...
				$datoscli = explode (":", $menu); // La función "explode" divide o separa una cadena en función de un delimitador.
				...
				$resulconsultapedidos = mysqli_query ($conexion, $sql1)
					or die ("Fallo en la consulta de pedidos");
				...
				if ($npedidos==0)
					...
					for ($fp=1; $fp<=$npedidos; $fp++) {
						...
						$resulconsultadetallespedido = mysqli_query ($conexion, $sql2)
							or die ("Fallo en la consulta de detallepedidos y productos");
			...
			// Sacamos menú de selección para elegir el cliente:
			$consulta = mysqli_query ($conexion, "select CodigoCliente,NombreCliente from clientes")
				or die ("Fallo en la consulta");
			$nfilas = mysqli_num_rows ($consulta);
			echo "<h1>Selecciona cliente a consultar</h1><br>";
			echo "<form  action='DWES ejercicio 3.1.8 (solución).php' method='get'>";
