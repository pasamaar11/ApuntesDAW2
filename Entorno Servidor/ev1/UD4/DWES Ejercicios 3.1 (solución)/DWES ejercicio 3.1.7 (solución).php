...
<html lang="es">
	...
	<body>
		...
		<nav></nav>
			<main>
				...
				?>
				<form action='<?= $_SERVER['PHP_SELF']?>' method='GET'>	<!-- Utilizando el array superglobal "$_SERVER". -->
					<label>Escribe el teléfono del cliente:</label>
					<input type='text' name='telefono' value='' pattern='[0-9]{9,11}' size='11' /> <!-- En vez de usar "required" se hace uso de "header(Location: )" en PHP. -->
	...
	function mostrarClienteyPreguntarBorrar($tel) {
		$conexion = mysqli_connect("localhost","jardinero","jardinero")
			or die ("Error en conexión con servidor bd");
		mysqli_select_db($conexion,"jardineria")
			or die ("Error al seleccionar bd jardinería");
		if (!empty($tel)) {
			...
			if ($fila==true) {
				...
				?>
				<form action='DWES ejercicio 3.1.7 (solución).php' method='get'>
				...
				echo "<p>El teléfono no pertenece a ningún cliente. <a href='DWES ejercicio 3.1.7 (solución).php'>VOLVER</a></p>";
			}
		} else {
			// Si el teléfono está vacío lo volvemos a pedir:
			header("Location: DWES ejercicio 3.1.7 (solución).php"); // Redirección del navegador.
		}
	}
	...
	function borrarCliente($codigo,$respuesta) {
		...
		if ($respuesta=="Si") {
			...
			mysqli_query($conexion, $query)
				or die ("Fallo al eliminar los detalles pedidos del cliente $codigo.");
		...
		echo "<p><a href='DWES ejercicio 3.1.7 (solución).php'>VOLVER</a></p>";
	}
	?>
